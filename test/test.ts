...
const knexFlavor = Knex({
  client: 'pg'
});

// searchPath
const knexSearch = Knex({
  client: 'pg',
  searchPath: 'public'
});
...
const subquery1 = knex('users').where('votes', '>', 100).andWhere('status', 'active').orWhere('name', 'John').select('id');
knex('accounts').where('id', 'in', subquery1);

knex.select('name').from('users')
  .whereIn('id', [1, 2, 3])
  .orWhereIn('id', [4, 5, 6]);

const subquery2 = knex.select('id').from('accounts');
knex.select('name').from('users')
  .whereIn('account_id', subquery2);
...
const someExternalMethod: Function = (id, trx) => {
  return 1;
};
...
const query: any = knex.from('test');
...
knex.select('name').from('users')
  ...
  .asCallback(function(err: any, rows: any[]) {
    ...
    knex.select('id').from('nicknames').whereIn('nickname', _.map<any, any>(rows, 'name'))
      .asCallback(function(err: any, rows: any[]) {
        if (err) return console.error(err);
        console.log(rows);
      });
...
// Retrieve the stream:
const stream1 = knex.select('*').from('users').stream();
const writableStream: NodeJS.ReadWriteStream = new stream.Duplex();
stream1.pipe(writableStream);

// With options:
const stream2 = knex.select('*').from('users').stream({highWaterMark: 5});
stream2.pipe(writableStream);

// Use as a promise:
(() => {

  const stream3 = knex.select('*').from('users').where(knex.raw('id = ?', [1])).stream(function(stream: any) {
...
const pipeStream = knex.select('*').from('users').pipe(writableStream);
const app: any = {
  log: (data) => null
};
